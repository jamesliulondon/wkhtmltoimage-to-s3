...
#author JamesLiu
RECIPIENTS="syseng@performgroup.com,hemna.juneja@performgroup.com"
...
{ 
  ...
  if [ -f ${TMP_DIR}/dash_${FILE_UID}.png ]; then 
    /bin/rm ${TMP_DIR}/dash_${FILE_UID}.png 
  fi 
#  if [ -f ${TMP_DIR}/dash_${FILE_UID}.png ]; then 
#    /bin/rm ${TMP_DIR}/dash_${FILE_UID}.png 
#  fi 
  if [ -f ${TMP_DIR}/index_${FILE_UID}.html ]; then 
    /bin/rm ${TMP_DIR}/index_${FILE_UID}.html 
  fi 
  exit $1 
} 
...
rm ${TMP_DIR}/dash_*.png

# Create HTML file 
cat <<EOF>${TMP_DIR}/index.${FILE_UID}.html
<html> 
<head> 
...
s3cmd put ${TMP_DIR}/index.${FILE_UID}.html s3://${S3_BUCKET}/index.${FILE_UID}.html
...
function create_dashboard_image() {
    wkhtmltoimage --quality 15  --zoom 0.75 'http://172.16.2.30/check_mk/view.py?view_name=svcproblems&_username=techsupport_mon&_secret=NPWTRFDJFQIHCMJAAVLG' ${TMP_DIR}/dash_${FILE_UID}.png
}

function upload_image() {
    create_dashboard_image
    s3cmd put ${TMP_DIR}/dash_${FILE_UID}.png s3://${S3_BUCKET}/dash_${FILE_UID}.png
}
